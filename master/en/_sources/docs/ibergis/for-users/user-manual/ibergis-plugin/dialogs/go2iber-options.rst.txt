...
- **Ignore quality:** allow choice if pollutant wash off, routing and treatment should be ignored in a project that has pollutants defined. The default is NO.
...
- **Sys flow tol:** maximum percent difference between total system inflow and total system outflow which can occur in order for the *Skip steady state* option to take effect. The default is 5 percent.
...
- **Minimum step:** the smallest time step allowed when variable time steps are used for dynamic wave flow routing. The default is 0.5.
...
Raster results symbology mode
-----------------------------

Sets the raster results symbology mode. The available options are:
  - **Min/Max per band:** automatically adjusts the color scale for each raster band individually using the minimum and maximum values detected within that band.
  - **Min/Max for all bands:** applies a single color scale to all raster bands, using the overall minimum and maximum values found across every band.
  - **Custom Min/Max:** uses custom minimum and maximum values defined in the :ref:`raster-configuration-for-each-layer` for all raster bands.

.. _raster-configuration-for-each-layer:

Raster configuration for each layer
-----------------------------------

Allows to configure the raster results for each layer. The following options are available:
  - **Maximum and minimum custom values:** sets the maximum and minimum values for the raster results.
  - **Symbology color ramp:** sets the symbology color ramp.
  - **Exclude values for the parameters:** allows you to exclude values to make the configuration easier.

The available layers to be configured are:
  - Depth
  - Velocity
  - Water Elevation
  - Hazard ACA
  - Severe Hazard RD9-2008
  - Local Time Step
